...
这篇笔记分三层：先用浏览器一手拆页面本身（数字截至 2026-09-17 中午，北京时间）；再结合罗福莉的官宣推文和量子位的报道，梳理这套直播背后的方法论；最后聊聊我的看法——这种"自信"到底是什么，以及它值不值。2026-09-19 凌晨与 09-20 晚各复访了一次直播页：前一次记在第十节，最新一次（flash 停跑、差距缩到 1.6 分）记在第十一节；09-21 的收官帧（两条 run 全部停跑、总账定格 347 万美元）见第十二节。
...
（第二天凌晨，pro 也停了——收官全记录见第十二节。）

## 十二、收官（2026-09-21）：两条 run 都停在 step 31 中途

> 2026-09-21 下午开直播页确认：**pro 也停了**。北京时间当天凌晨 1 点 47 分，pro 跑完 step 30（dynsam/avg\@n 收在 0.633，对比 step 1 +0.068）；凌晨 2 点 02 分，feed 落下最后一条 "run stopped"——step 31 进行到 664/1,568。两条 run 以完全相同的方式谢幕：**最后完成的都是 step 30，都停在 step 31 中途**。

![收官帧（09-21）：计数器定格 \$3,474,715，两条 run 均为 stopped（pro 09-20、flash 09-19），三张基准图定格终值](/assets/images/2026/20260921/mimo-rl-final.webp)

**总账定格**：total cost **\$3,474,715**（pro \$2,620,670 + flash \$854,044，差 \$1 属计数器秒级跳动）——我 09-20 晚抓帧之后，pro 又烧了最后 **\$9.3 万**。整场直播从 09-16 中午开播到 09-21 凌晨收官，历时 **4 天 14 小时**；两条 run 合计处理 **1,560 亿 token、训练 150.6 万个样本**。

**终盘数据**（avg\@n 均对比 step 1）：

| | mimo-v2.6-pro | mimo-v2.6-flash |
| --- | --- | --- |
| 最终 step | 30（停在 31 中途） | 30（停在 31 中途） |
| dynsam/avg\@n | 0.633（+0.068） | 0.644（+0.130） |
| 累计 token | 750 亿 | 814 亿 |
| 已训练样本 | 75.3 万 | 75.3 万 |
| 总花费 | \$2,620,670 | \$854,044 |

连"已训练样本"都停在一模一样的 **75.3 万**——两条 run 的谢幕对称得像是排练过的。最终配方（step 30）：code 65.8%、visual 19.5%、general 11.3%、chat 3.4%、cyber 0%。

**基准终值**：DeepSWE v1.1 **pro 72.57 / flash 65.68**（与 DeepSeek-Flash 74.2 的最终差距 **1.63 分**）；In-house Coding **65.43 / 62.87**；AutomationBench **53.10 / 52.70**。后两项在停跑之后还小幅上修过（pro 分别 +1.04、+1.80 分）——离线评测在训练结束后仍在后台跑。

**收官方式与 flash 一致：没有任何公告**。第 6 条"过滤简单任务"就是公告栏的最后一条；两条 run 为什么停、是预算到点、练到目标、还是版本冻结等发布，官方一字未提，about 页那句 "coming soon" 也原样挂着。答案大概率要等罗福莉承诺的"未来几周开源"和 MiMo-V2.6 正式发布。

**五天半直播的完整清单**：总花费 \$347 万；pro 三次重启（显存故障、grader 网络故障、expert 负载不均致 GPU OOM）+ flash 一次重启（infra 错误 3 小时未检出）；一次数据集当众移除（cyber）；一次难度动态过滤；六条公告零删改；基准差距 10.5 → 6.7 → 1.6 分；两条 run 同样收在 step 31 中途。

最后回到开篇那个问题——"这是要怎样的自信"。直播五天半看下来，答案已经不止"自信"二字：**这是一次把自信本身做成展示品的尝试**。系统当众崩过四次、配本当众改过两次、钱当众烧了 347 万、落后当众追到 1.6 分，全程没删过一条公告、没关过一次直播间。无论 MiMo-V2.6 正式发布时成绩如何，这场直播本身已经是个值得记下的行业事件：它把"过程透明"的边界，从开源代码和事后报告，推进到了**正在烧钱的训练现场**。第九节留的三个悬念现在能销掉一半——差距追到了 1.6 分；Credit Assignment 是否如期开源、有没有实验室跟进"训练直播"，继续等着看下文。
