...
$C$是真实值中的簇的总数，$N_c$簇$c$中的元素数量，$x_i$是一个元素对应的嵌入向量，$\mu_c$是&c&簇中嵌入向量的平均值（簇的中心），||·||是L1距离或者L2距离，$[x]_+$是max(0,x)定义的铰链函数，$\delta_v$和$\delta_d$代表方差和距离损失的间隙。
...
$$L_{dist}=\frac{1}{C(C-1)}\underbrace{\sum_{c_A=1}^{C}\sum_{c_B=1}^{C}}_{{c_A}\neq{c_B}}[2\delta_d-||{\mu_{cA}-\mu_{cB}}||]_+^2$$
